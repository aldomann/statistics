...
```
The RSS for the test data using a Ridge regression is `r apps.ridge.test.rss`, which is a bit higher than the one obtained using OLS.
...
The RSS for the test data using a Lasso regression is `r apps.lasso.test.rss`, which is again a bit higher than the one obtanes using OLS, and somewhat smaller than the RSS obtained using a Ridge regression.
...
The plot shows that test $R^2$ for all models are around 0.9, with OLS having slightly higher test $R^2$ than Ridge and Lasso. We can conclude that all models predict college applications with high accuracy. 
