...
As we can see from the results, with $C_{p}$, $BIC$, and adjusted $R^{2}$ criteria, 3-variable models are picked. The expression for this 3-variable model is the following:
```{r}
...
```{r}
plot_statistics(fwd.summary, "Cp", min.cp.fwd, "Forward Stepwise")
plot_statistics(fwd.summary, "BIC", min.bic.fwd, "Forward Stepwise")
plot_statistics(fwd.summary, "AdjR2", max.adjr2.fwd, "Forward Stepwise")

plot_statistics(bwd.summary, "Cp", min.cp.bwd, "Backward Stepwise")
plot_statistics(bwd.summary, "BIC", min.bic.bwd, "Backward Stepwise")
plot_statistics(bwd.summary, "AdjR2", max.adjr2.bwd, "Backward Stepwise")
```

As we can see from the results, using the backward stepwise method with $C_{p}$, $BIC$, and adjusted $R^{2}$ criteria, 3-variable models are picked, as it was with the exhaustive method. Again, all three statistics pick $X^{5}$ over $X^{3}$.

For the forward stepwise model, though, for the $C_{p}$, $BIC$, and the adjusted $R^{2}$ criteria we get a 4, 3, and 4-variable model, respectively. The 4-variable model has the following expression:
```{r}
coefficients(mod.fwd, id = min.cp.fwd)
```

In this case, instead of picking $X^{5}$ over $X^{3}$, $X^{5}$ is an additional term to the selected predictors of the model.
...
Now, the final step is to use the `predict()` function with the lasso model:
```{r}
