...
As we can see from the results, with $C_{p}$, $BIC$, and adjusted $R^{2}$ criteria, 4, 3, 4-variable models are picked, respectively. The expression for this 4-variable and 3-variable model are the following, respectively:
```{r}
# 4-variable model
...
Let's notice that lasso predicts $\beta_{0} + \beta_{1}X + \beta_{2}{X}^{2} + \beta_{3}{X}^{3} + \beta_{10}{X}^{10}$, but $\beta_{10}$ is almost negligible.
...
mod2.full <- regsubsets(Y2 ~ poly(X, 10, raw = T), 
												data = tibble(Y2, X), 
												nvmax = 10)
mod2.summary <- summary(mod2.full)
```
...
(g) Create a plot displaying $\sqrt{\sum_{j=1}^{p}{(\beta_{j} -\hat{\beta}_{j}^{r})}^{2}}$ for a range of values of $r$, where $\hat{\beta}_{j}^{r}$ is the jth coefficient estimate for the best model containing $r$ coefficients. Comment on what you observe. How does this compare to the test MSE plot from (d)?
