...
Now we will perform a linear model for the number of applications (`Apps`) and calculate the RSS for the test data:
```{r}
apps.fit <- lm(Apps ~ ., data = college.train)
apps.pred <- predict(apps.fit, newdata = college.test)
apps.test.rss <- mean((college.test$Apps - apps.pred)^2)
```
The RSS for the test data is `r apps.test.rss`.
